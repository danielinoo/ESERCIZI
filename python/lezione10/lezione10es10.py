...
def memorizza_file(files: list[int]) -> None:
    ...
    blocco  : int= 512
    blocchi_usati  : int= 0
    compressione : float = 0

    for i in files:

        blocchi_usati = i // blocco

        compressione = (i * 80) /100

        spazio_totale_blocchi = spazio_totale_blocchi - blocchi_usati

        if blocchi_usati <= spazio_totale_blocchi:

            print(f"File di {i} byte compresso in {compressione} byte e memorizzato. Blocchi usati: {blocchi_usati}. Blocchi rimanenti: {spazio_totale_blocchi}.")
